...
/// @brief A specialized stream for buton presses (not tricks).
...
/// Bitmask  | Description
///------------- | -------------
/// 0x0F  | Increments when held every 255 frames*
/// 0x70  | 0x00 = No trick, 0x10 = Up/Wheelie, 0x20 = Down, 0x30 = Left, 0x40 = Right
/// \*0x0F bitmask is combined with the tuple duration byte to determine trick duration.
/// The belief here is that tricking occurs infrequently enough that it's better to reserve an extra
/// 4 bits to allow tuples to represent a longer duration.
...
protected:
    ...
    RaceInputState m_lastInputState; ///< Used to determine if we have just started drifting, e.g.
};
