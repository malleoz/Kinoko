...
MapdataAreaBox::MapdataAreaBox(const SData *data) : MapdataArea(data) {
    ...
    m_boundingCircleRadius = m_dims.dot();
...
/// @brief Override because raw SData size varies depending on course version.
...
void MapdataAreaAccessor::init(const MapdataArea::SData *start, u16 count) {
    ...
    bool isOldVer = CourseMap::Instance()->version() < 0x898;

    for (u16 i = 0; i < count; ++i) {
        const MapdataArea::SData *data = &start[i];

        if (isOldVer) [[unlikely]] {
            data = reinterpret_cast<const MapdataArea::SData *>(
                    reinterpret_cast<uintptr_t>(&start[i]) - 4 * i);
        }
