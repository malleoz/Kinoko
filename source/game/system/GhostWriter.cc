...
GhostWriter::GhostWriter() {
    ...
    memset(m_buffer.data(), 0, TRIPLE_RKG_BUFFER_SIZE);
...
bool GhostWriter::saveToBuffer(EGG::RamStream &outStream) {
    memset(outStream.data(), 0, 8);

    // First write the counts
    u32 offset = 8;
    for (u32 i = 0; i < m_buttonsStreams.size(); ++i) {
        // Jump back so we write the next sequence count
        outStream.jump(i * sizeof(u16));

        auto *&stream = m_buttonsStreams[i];
        u16 count = static_cast<u16>(stream->sequenceCount) + 1;
        outStream.write_u16(count);
        outStream.jump(offset);
        outStream.write(stream->buffer.data(), count * KPadGhostButtonsStream::SEQUENCE_SIZE);
        offset += count * KPadGhostButtonsStream::SEQUENCE_SIZE;
    }
