...
#include <abstract/OSCalendarTime.hh>
...
GhostFile::GhostFile(u32 /*playerIdx*/) {
    ...
    time.year = std::min<u32>(99, time.year);

    m_year = static_cast<u8>(time.year);
    m_month = static_cast<u8>(time.month + 1);
    m_day = static_cast<u8>(time.day);
    m_location = 0;
