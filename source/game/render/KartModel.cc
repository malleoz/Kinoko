...
void KartModel::vf_1c() {
    const Kart::KartStatus &status = KartObjectProxy::status();

    if (status.onBit(Kart::eKartStatus::Burnout)) {
        _54 = 1.0f;
    ...
    bool isInCannon = status.onBit(Kart::eKartStatus::InCannon);
...
void KartModel::FUN_807CB198() {
    m_somethingRight = false;
    m_somethingLeft = false;
    Kart::KartStatus &status = KartObjectProxy::status();

    bool turnInput = status.onBit(Kart::eKartStatus::StickLeft, Kart::eKartStatus::StickRight);
    if (state()->isDrifting() || (status.onBit(Kart::eKartStatus::ChargingSSMT) && turnInput)) {
        if (move()->hopStickX() == 1) {
            m_somethingLeft = true;
        } else {
            if (move()->hopStickX() == -1) {
                m_somethingRight = true;
            } else if (status.offBit(Kart::eKartStatus::StickLeft)) {
                m_somethingRight = true;
            } else {
                m_somethingLeft = true;
            }
