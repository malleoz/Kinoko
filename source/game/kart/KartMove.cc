...
s32 KartMove::appliedHopStickX() const {
...
/// @addr{0x8057C3C8}
f32 KartMove::wheelieSoftSpeedLimitBonus() const {
    return 0.0f;
}
...
void KartMove::calcManualDrift() {
    ...
    if (!state()->isTouchingGround() &&
        ...
        if (driftRej.normalise() != 0.0f) {
            const f32 rejCrossDirMag = driftRej.cross(rotZ).length();
            const f32 angle = EGG::Mathf::atan2(rejCrossDirMag, driftRej.dot(rotZ));
            f32 sign = 1.0f;
    ...
    if (!state()->isDriftManual()) {
        if (!isHopping && state()->isTouchingGround()) {
            ...
            const f32 driftAngleDecr = param()->stats().driftOutsideDecrement;
...
void KartMove::controlOutsideDriftAngle() {
    ...
    if (param()->stats().driftType != KartParam::Stats::DriftType::Inside_Drift_Bike) {
        if (m_hopStickX == -1) {
            ...
            const f32 targetAngle = param()->stats().driftOutsideTargetAngle;
...
void KartMove::calcAcceleration() {
    ...
    dVar17 *= (m_boost.multiplier() + wheelieSoftSpeedLimitBonus()) * m_kclSpeedFactor;
...
/// @addr{0x80588324}
f32 KartMoveBike::wheelieSoftSpeedLimitBonus() const {
