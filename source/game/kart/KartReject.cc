...
void KartReject::calcRejectRoad() {
    ...
    if (status.onBit(eKartStatus::RejectRoadTrigger)) {
        ...
        status.resetBit(eKartStatus::Hop);

        bool didReject = calcRejection();

        if (status.offBit(eKartStatus::NoSparkInvisibleWall) && !didReject) {
            status.resetBit(eKartStatus::RejectRoadTrigger);
        }

        return;
    }

    if (status.onBit(eKartStatus::RejectRoad) &&
            status.offBit(eKartStatus::ZipperInvisibleWall, eKartStatus::OverZipper,
        ...
        if (upXZ.length() > 0.0f && speed() > 0.0f) {
            ...
            if (local_88.y > 0.0f) {
                ...
                status.resetBit(eKartStatus::Hop).setBit(eKartStatus::RejectRoadTrigger);
            }
        }
    }
}
...
bool KartReject::calcRejection() {
    ...
    KartStatus &status = KartObjectProxy::status();
    status.resetBit(eKartStatus::NoSparkInvisibleWall);
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        if (hasInvisibleWallCollision && KCL_VARIANT_TYPE(closestColEntry->attribute) == 0) {
            ...
            status.setBit(eKartStatus::NoSparkInvisibleWall);
        } else {
        ...
        if (bVar15 || extVel().y < 0.0f || status.onBit(eKartStatus::NoSparkInvisibleWall)) {
            radius = -radius;
