...
void WheelPhysics::updateCollision(const EGG::Vector3f &bottom, const EGG::Vector3f &topmostPos) {
    ...
    if (state()->flags().offBit(KartState::eFlag::SkipWheelCalc)) {
        ...
        if (state()->flags().onBit(KartState::eFlag::InCannon)) {
            ...
            if (state()->flags().onBit(KartState::eFlag::UNK2)) {
...
void KartSuspensionPhysics::calcSuspension(const EGG::Vector3f &forward,
    ...
    if (dynamics()->extVel().y > 5.0f ||
            state()->flags().onBit(KartState::eFlag::JumpPadDisableYsusForce)) {
        fLinear.y = 0.0f;
    }

    dynamics()->applySuspensionWrench(m_topmostPos, fLinear, fRot,
            state()->flags().onBit(KartState::eFlag::WheelieRot));

    f32 rate = state()->flags().onBit(KartState::eFlag::SomethingWallCollision) ?
            0.01f :
            collide()->floorMomentRate();

    collide()->applySomeFloorMoment(0.1f, rate, hitboxGroup, forward, move()->dir(),
            m_tirePhysics->speed(), true, true,
            state()->flags().offBit(KartState::eFlag::WheelieRot));
}
