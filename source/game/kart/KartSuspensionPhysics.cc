...
void WheelPhysics::updateCollision(const EGG::Vector3f &bottom, const EGG::Vector3f &topmostPos) {
    ...
    if (status.offBit(eKartStatus::SkipWheelCalc)) {
        ...
        if (status.onBit(eKartStatus::InCannon)) {
            ...
            if (status.onBit(eKartStatus::UNK2)) {
...
void KartSuspensionPhysics::calcSuspension(const EGG::Vector3f &forward,
    ...
    if (dynamics()->extVel().y > 5.0f || status.onBit(eKartStatus::JumpPadDisableYsusForce)) {
        fLinear.y = 0.0f;
    }

    bool wheelieRot = status.onBit(eKartStatus::WheelieRot);

    dynamics()->applySuspensionWrench(m_topmostPos, fLinear, fRot, wheelieRot);

    f32 rate = status.onBit(eKartStatus::SomethingWallCollision) ? 0.01f :
