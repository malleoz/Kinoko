...
public:
    ...
    virtual bool canHop() const {
        if (state()->flags().offBit(KartState::eFlag::HopStart) ||
...
public:
    ...
    [[nodiscard]] f32 getWheelieSoftSpeedLimitBonus() const override {
        ...
        return state()->flags().onBit(KartState::eFlag::Wheelie) ? WHEELIE_SPEED_BONUS : 0.0f;
    }
    ...
    [[nodiscard]] f32 wheelieRotFactor() const {
        ...
        return state()->flags().onBit(KartState::eFlag::Wheelie) ? WHEELIE_ROTATION_FACTOR : 1.0f;
    }
