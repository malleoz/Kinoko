...
void KartCollide::findCollision() {
    bool wasHalfPipe = status().onBit(eKartStatus::EndHalfPipe, eKartStatus::ActionMidZipper);
...
void KartCollide::FUN_80572F4C() {
    ...
    if (isInRespawn() ||
            status.onBit(eKartStatus::Boost, eKartStatus::OverZipper,
                    eKartStatus::ZipperInvisibleWall, eKartStatus::NoSparkInvisibleWall,
    ...
    bool resetXZ = fVar1 > 0.0f && status.onBit(eKartStatus::AirtimeOver20) &&
            dynamics()->velocity().y < -50.0f;

    FUN_805B72B8(status.onBit(eKartStatus::InAction) ? 0.3f : 0.01f, fVar1, resetXZ,
            status.offBit(eKartStatus::JumpPadDisableYsusForce));
}
    ...
    if (*mask & KCL_TYPE_BIT(COL_TYPE_EFFECT_TRIGGER)) {
        ...
        if (colDir->findClosestCollisionEntry(mask, KCL_TYPE_BIT(COL_TYPE_EFFECT_TRIGGER))) {
            if (KCL_VARIANT_TYPE(colDir->closestCollisionEntry()->attribute) == 4) {
                halfPipe()->end(true);
                status().setBit(eKartStatus::EndHalfPipe);
...
void KartCollide::calcBeforeRespawn() {
    ...
    if (status.offBit(eKartStatus::BeforeRespawn)) {
        return;
    }
    ...
    status.resetBit(eKartStatus::BeforeRespawn);
    m_respawnTimer = 0;
    move()->triggerRespawn();
}
...
void KartCollide::activateOob(bool /*detachCamera*/, Field::KCLTypeMask * /*mask*/,
    ...
    status.setBit(eKartStatus::BeforeRespawn);
}
...
void KartCollide::calcPoleTimer() {
    if (m_poleAngVelTimer > 0 && status().onBit(eKartStatus::Accelerate, eKartStatus::Brake)) {
...
void KartCollide::processFloor(CollisionData &collisionData, Hitbox &hitbox,
    ...
    if (attribute & 0x4000) {
        status.setBit(eKartStatus::RejectRoad);
    }
    ...
    if (*maskOut & KCL_TYPE_BIT(COL_TYPE_STICKY_ROAD)) {
        status.setBit(eKartStatus::StickyRoad);
    }
    ...
    if ((*maskOut & halfPipeRampMask) &&
        ...
        status.setBit(eKartStatus::HalfPipeRamp);
    ...
    if (*maskOut & jumpPadMask && colDirector->findClosestCollisionEntry(maskOut, jumpPadMask)) {
        if (status.offAnyBit(eKartStatus::TouchingGround, eKartStatus::JumpPad) &&
                status.offBit(eKartStatus::JumpPadMushroomVelYInc)) {
...
void KartCollide::processCannon(Field::KCLTypeMask *maskOut) {
    ...
    if (colDirector->findClosestCollisionEntry(maskOut, KCL_TYPE_BIT(COL_TYPE_CANNON_TRIGGER))) {
        ...
        status().setBit(eKartStatus::CannonStart);
    }
}
...
bool KartCollide::FUN_805B6A9C(CollisionData &collisionData, const Hitbox &hitbox,
    ...
    if (maskOut & KCL_TYPE_WALL) {
        if (!(maskOut & KCL_TYPE_FLOOR) && status().onBit(eKartStatus::HWG) &&
...
void KartCollide::calcFloorMomentRate() {
    m_floorMomentRate = status().onBit(eKartStatus::InAction) &&
