...
void KartCollide::findCollision() {
    const EGG::Quatf &rot =
            state()->flags().onBit(KartState::eFlag::EndHalfPipe) ? mainRot() : fullRot();
...
void KartCollide::FUN_80572F4C() {
    ...
    if (isInRespawn() || state()->flags().onBit(KartState::eFlag::Boost) ||
            state()->flags().onBit(KartState::eFlag::OverZipper) ||
            state()->flags().onBit(KartState::eFlag::ZipperInvisibleWall) ||
    ...
    bool resetXZ = fVar1 > 0.0f && state()->flags().onBit(KartState::eFlag::AirtimeOver20) &&
            dynamics()->velocity().y < -50.0f;

    FUN_805B72B8(state()->flags().onBit(KartState::eFlag::InAction) ? 0.3f : 0.01f, fVar1, resetXZ,
            state()->flags().offBit(KartState::eFlag::JumpPadDisableYsusForce));
}
    ...
    if (*mask & KCL_TYPE_BIT(COL_TYPE_EFFECT_TRIGGER)) {
        ...
        if (colDir->findClosestCollisionEntry(mask, KCL_TYPE_BIT(COL_TYPE_EFFECT_TRIGGER))) {
            if (KCL_VARIANT_TYPE(colDir->closestCollisionEntry()->attribute) == 4) {
                halfPipe()->end(true);
                state()->flags().setBit(KartState::eFlag::EndHalfPipe);
...
void KartCollide::calcBeforeRespawn() {
    ...
    state()->flags().resetBit(KartState::eFlag::BeforeRespawn);
    m_respawnTimer = 0;
    move()->triggerRespawn();
}
...
void KartCollide::activateOob(bool /*detachCamera*/, Field::KCLTypeMask * /*mask*/,
    ...
    state()->flags().setBit(KartState::eFlag::BeforeRespawn);
}
...
void KartCollide::calcPoleTimer() {
    if (m_poleAngVelTimer > 0 &&
            (state()->flags().onBit(KartState::eFlag::Accelerate) ||
                    state()->flags().onBit(KartState::eFlag::Brake))) {
...
void KartCollide::processFloor(CollisionData &collisionData, Hitbox &hitbox,
    ...
    if (attribute & 0x4000) {
        state()->flags().setBit(KartState::eFlag::RejectRoad);
    }
    ...
    if (*maskOut & KCL_TYPE_BIT(COL_TYPE_STICKY_ROAD)) {
        state()->flags().setBit(KartState::eFlag::StickyRoad);
    }
    ...
    if ((*maskOut & halfPipeRampMask) &&
        ...
        state()->flags().setBit(KartState::eFlag::HalfPipeRamp);
...
void KartCollide::processCannon(Field::KCLTypeMask *maskOut) {
    ...
    if (colDirector->findClosestCollisionEntry(maskOut, KCL_TYPE_BIT(COL_TYPE_CANNON_TRIGGER))) {
        ...
        state()->flags().setBit(KartState::eFlag::CannonStart);
    }
}
...
bool KartCollide::FUN_805B6A9C(CollisionData &collisionData, const Hitbox &hitbox,
    ...
    if (maskOut & KCL_TYPE_WALL) {
        if (!(maskOut & KCL_TYPE_FLOOR) && state()->flags().onBit(KartState::eFlag::HWG) &&
...
void KartCollide::calcFloorMomentRate() {
    m_floorMomentRate = state()->flags().onBit(KartState::eFlag::InAction) ?
            0.01f :
