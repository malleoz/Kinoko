...
KartState::KartState() {
    ...
    m_status.changeBit(inputs()->driftIsAuto(), eKartStatus::AutoDrift);
...
void KartState::reset() {
    ...
    bool isAutoDrift = m_status.onBit(eKartStatus::AutoDrift);
    m_status.makeAllZero().changeBit(isAutoDrift, eKartStatus::AutoDrift);
...
void KartState::calcInput() {
    ...
    if (raceMgr->isStageReached(System::RaceManager::Stage::Race)) {
        if (m_status.offBit(eKartStatus::InAction, eKartStatus::BeforeRespawn,
                    eKartStatus::CannonStart, eKartStatus::InCannon, eKartStatus::OverZipper)) {
            ...
            if (m_status.offBit(eKartStatus::RejectRoadTrigger)) {
                if (m_stickX < 0.0f) {
                    m_status.setBit(eKartStatus::StickLeft);
                } else if (m_stickX > 0.0f) {
                    m_status.setBit(eKartStatus::StickRight);
                }
            }

            if (m_status.offBit(eKartStatus::Burnout)) {
                m_status.changeBit(currentState.accelerate(), eKartStatus::Accelerate)
                        .changeBit(currentState.accelerate() && !lastState.accelerate(),
                                eKartStatus::AccelerateStart)
                        .changeBit(currentState.brake(), eKartStatus::Brake);

                if (m_status.offBit(eKartStatus::AutoDrift)) {
                    m_status.changeBit(currentState.drift(), eKartStatus::DriftInput)
                            .changeBit(currentState.drift() && !lastState.drift(),
                                    eKartStatus::HopStart);
                }
            }
        }
        ...
        const auto &currentState = inputs()->currentState();
        m_stickX = currentState.stick.x;
        m_status.changeBit(currentState.accelerate(), eKartStatus::ChargeStartBoost);
...
void KartState::resetFlags() {
    m_status.resetBit(eKartStatus::Accelerate, eKartStatus::Brake, eKartStatus::DriftInput,
            eKartStatus::HopStart, eKartStatus::AccelerateStart, eKartStatus::GroundStart,
            eKartStatus::StickLeft, eKartStatus::WallCollisionStart, eKartStatus::AirStart,
            eKartStatus::StickRight, eKartStatus::ZipperInvisibleWall,
            eKartStatus::JumpPadDisableYsusForce);
...
void KartState::calcCollisions() {
    bool wasTouchingGround = m_status.onBit(eKartStatus::TouchingGround);
    bool wasWallCollision = m_status.onBit(eKartStatus::WallCollision, eKartStatus::Wall3Collision);

    m_status.resetBit(eKartStatus::Wall3Collision, eKartStatus::WallCollision,
            eKartStatus::VehicleBodyFloorCollision, eKartStatus::AnyWheelCollision,
            eKartStatus::AllWheelsCollision, eKartStatus::TouchingGround);

    if (m_hwgTimer > 0) {
        if (--m_hwgTimer == 0) {
            m_status.resetBit(eKartStatus::UNK2, eKartStatus::SomethingWallCollision);
        }
    }
    ...
    if (collide()->someSoftWallTimer() > 0) {
        if (collide()->someNonSoftWallTimer() == 0) {
            ...
            if (softSusp - nonSusp >= 40.0f) {
                m_status.resetBit(eKartStatus::SoftWallDrift);
            } else {
                softWallCollision = true;
            }
        }
    ...
    if (wheelCollisions > 0) {
        m_status.setBit(eKartStatus::AnyWheelCollision);
        if (wheelCollisions == tireCount()) {
            m_status.setBit(eKartStatus::AllWheelsCollision);
        }
    }

    CollisionData &colData = collisionData();
    if (colData.bFloor) {
        m_status.setBit(eKartStatus::VehicleBodyFloorCollision);
        m_top += colData.floorNrm;
        trickable = trickable || colData.bTrickable;

        if (m_status.onBit(eKartStatus::OverZipper)) {
            halfPipe()->end(true);
        }
    }
    ...
    if ((colData.bWall || colData.bWall3) && !bVar3) {
        if (colData.bWall) {
            status().setBit(eKartStatus::WallCollision);
        }

        if (colData.bWall3) {
            m_status.setBit(eKartStatus::Wall3Collision);
        }

        if (!wasWallCollision) {
            m_status.setBit(eKartStatus::WallCollisionStart);
        }
    ...
    if (colData.bInvisibleWall && m_status.onBit(eKartStatus::HalfPipeRamp) &&
            collide()->surfaceFlags().offBit(KartCollide::eSurfaceFlags::StopHalfPipeState)) {
        m_status.setBit(eKartStatus::ZipperInvisibleWall);
    }

    if (softWallCount > 0 || hwg) {
        m_status.setBit(eKartStatus::UNK2);
        m_softWallSpeed = wallNrm;
        m_softWallSpeed.normalise();
        if (softWallCount > 0 && m_status.offBit(eKartStatus::Hop)) {
            m_status.setBit(eKartStatus::SoftWallDrift);
        }

        if (hwg) {
            m_status.setBit(eKartStatus::HWG);
        }

        if (hitboxGroupSoftWallCollision || hwg || isBike()) {
            m_status.setBit(eKartStatus::SomethingWallCollision);
            m_hwgTimer = 10;
    ...
    m_status.resetBit(eKartStatus::AirtimeOver20);
    m_trickableTimer = std::max(0, m_trickableTimer - 1);

    if (wheelCollisions < 1 && !colData.bFloor) {
        if (wasTouchingGround) {
            m_status.setBit(eKartStatus::AirStart);
        }

        if (++m_airtime > 20) {
            m_status.setBit(eKartStatus::AirtimeOver20);
        }
    } else {
        m_top.normalise();

        m_status.setBit(eKartStatus::TouchingGround).resetBit(eKartStatus::AfterCannon);

        if (m_status.offBit(eKartStatus::InAction)) {
            m_status.resetBit(eKartStatus::ActionMidZipper, eKartStatus::EndHalfPipe);
        }
        ...
        m_status.changeBit(m_trickableTimer > 0, eKartStatus::Trickable);

        if (m_status.offBit(eKartStatus::JumpPad)) {
            m_status.resetBit(eKartStatus::JumpPadMushroomCollision);
        }

        if (!wasTouchingGround) {
            m_status.setBit(eKartStatus::GroundStart);
        }

        if (m_status.onBit(eKartStatus::InATrick) && jump()->cooldown() == 0) {
            move()->landTrick();
...
void KartState::calcStartBoost() {
    ...
    if (m_status.onBit(eKartStatus::ChargeStartBoost)) {
...
void KartState::calcHandleStartBoost() {
    ...
    if (m_status.onBit(eKartStatus::Accelerate)) {
    ...
    m_status.resetBit(eKartStatus::ChargeStartBoost);
}
...
void KartState::resetEjection() {
    m_status.resetBit(eKartStatus::HalfPipeRamp, eKartStatus::RejectRoad);
}
