...
void ObjectTruckWagonCart::calc() {
    ...
    EGG::Vector3f vel = m_railInterpolator->currVel() * m_railInterpolator->curTangentDir();
    m_vel = EGG::Vector3f(vel.x, m_vel.y, vel.z);
...
void ObjectTruckWagonCart::calcCollisionTransform() {
    ...
    f32 idk = m_currentStateId == 1 ? 500.0f : 100.0f;

    EGG::Matrix34f mat = EGG::Matrix34f::zero;
    mat.makeT(EGG::Vector3f(0.0f, idk, 0.0f));
...
void ObjectTruckWagonCart::calcState0() {
    ...
    m_pos = EGG::Vector3f(railPos.x, m_pos.y - 15.0f, railPos.z);
    m_flags.setBit(eFlags::Position);
    ...
    setMatrixTangentTo(m_up, m_tangent);
} // 2615 pos.y wrong by 30

/// @addr{0x806E1370}
void ObjectTruckWagonCart::calcState1() {
    EGG::Vector2f xz_f0 = EGG::Vector2f(m_lastVel.x, m_lastVel.z);
    EGG::Vector2f xz_vel = EGG::Vector2f(m_vel.x, m_vel.z);
    EGG::Vector2f diff = xz_vel - xz_f0;

    f32 sqrt = EGG::Mathf::sqrt(diff.dot());
    f32 dVar5;

    if (xz_vel.cross(xz_f0) > 0.0f) {
        dVar5 = -sqrt * EGG::Mathf::CosFIdx(RAD2FIDX * m_pitch);
    } else {
        dVar5 = sqrt * EGG::Mathf::CosFIdx(RAD2FIDX * m_pitch);
    }

    f32 dVar5_2 = EGG::Mathf::SinFIdx(RAD2FIDX * m_pitch);
    m_angVel = (m_angVel + (dVar5 + -3.9f * dVar5_2) / 1300.0f) * 0.998f;
    m_pitch += m_angVel;

    EGG::Vector3f xzTan = m_railInterpolator->curTangentDir();
    xzTan.y = 0.0f;
    xzTan.normalise2();
    EGG::Matrix34f mat;
    mat.setAxisRotation(m_pitch, xzTan);
    mat.setBase(3, EGG::Vector3f::zero);

    m_up = Interpolate(0.1f, m_up, mat.ps_multVector(EGG::Vector3f::ey));
    m_tangent = Interpolate(0.1f, m_tangent, xzTan);
    m_tangent.y = 0.0f;
    ...
    EGG::Vector3f local_bc = EGG::Vector3f(0.0f, 710.0f, 0.0f);
    EGG::Vector3f vStack_c8 = mat.ps_multVector(local_bc);
    m_flags.setBit(eFlags::Position);
    m_pos = m_railInterpolator->curPos() + local_bc - vStack_c8;
    m_lastVel = m_vel;
}
...
ObjectTruckWagon::ObjectTruckWagon(const System::MapdataGeoObj &params)
    : ObjectCollidable(params), m_spawn1Frame(static_cast<s32>(params.setting(1))),
      m_spawn2Frame(static_cast<s32>(params.setting(2))) {
...
void ObjectTruckWagon::calc() {
    ...
    if (m_cycleFrame == m_spawn1Frame || m_cycleFrame == m_spawn1Frame + m_spawn2Frame) {
        m_carts[m_curCartIdx]->activate();
        m_curCartIdx = (m_curCartIdx + 1) % m_carts.size();
    }

    m_cycleFrame = m_cycleFrame % (m_spawn1Frame + m_spawn2Frame) + 1;
}
