...
void ObjectPropeller::init() {
    ASSERT(m_mapObj);
    m_b4 = static_cast<f32>(static_cast<s16>(m_mapObj->setting(0)));
    ...
    m_128 = m_rotMat;
    m_128.setBase(3, m_pos + EGG::Vector3f::ey * 100.0f);
}
...
void ObjectPropeller::calcCollisionTransform() {
    ...
    for (u32 i = 0; i < m_blades.size(); ++i) {
        ...
        mat.setAxisRotation(static_cast<f32>(angle) * DEG2RAD, m_axis);
        calcTransform();
        EGG::Matrix34f mStack_b0 = mat.multiplyTo(m_transform);
        EGG::Vector3f local_bc = mStack_b0.base(1);
        local_bc.normalise();
        local_bc *= 250.0f;
        mStack_b0.setBase(3, m_pos + local_bc);

        auto *&blade = m_blades[i];
        blade->transform(mStack_b0, m_scale);
