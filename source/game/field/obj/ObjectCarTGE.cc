...
/// @addr{0x808C5860}
static f32 CAR_SCALE;

/// @addr{0x806D5EE4}
ObjectCarTGE::ObjectCarTGE(const System::MapdataGeoObj &params)
    : ObjectCollidable(params), StateManager(this), m_up(EGG::Vector3f::zero),
      m_tangent(EGG::Vector3f::zero) {
    m_carVariant = params.setting(3);
    m_vel0 = params.setting(2);
    m_vel1 = params.setting(1);

    // In the base game, we early return before the StateManager sets its state entries.
    ...
    // We also need to zero-initialize some members to match base game behavior.
    if (params.pathId() == -1) {
        m_nextStateId = -1;
        m_currentStateId = 0;
        m_currentFrame = 0;
        ...
        m_highwayMgr = nullptr;
        m_auxCollision = nullptr;
        memset(m_carName, 0, sizeof(m_carName));
        memset(m_mdlName, 0, sizeof(m_mdlName));
        m_carType = CarType::Normal;
        m_dummyId = static_cast<ObjectId>(0);
        m_initialScale = 0.0f;
        m_currSpeed = 0.0f;
        m_scaledTangentDir.setZero();
        m_curFloorNrm.setZero();
        m_nextFloorNrm.setZero();
    ...
    case 0: {
        if (strcmp(resourceName, "K_truck") == 0) {
            snprintf(m_mdlName, sizeof(m_mdlName), "%s_b", resourceName);
    ...
    case 1: {
        if (strcmp(resourceName, "K_truck") == 0) {
            snprintf(m_mdlName, sizeof(m_mdlName), "%s_o", resourceName);
        } else if (strcmp(resourceName, "K_car_body") == 0) {
    ...
    case 2: {
        if (strcmp(resourceName, "K_truck") == 0) {
            snprintf(m_mdlName, sizeof(m_mdlName), "%s_g", resourceName);
        } else if (strcmp(resourceName, "K_car_body") == 0) {
            snprintf(m_mdlName, sizeof(m_mdlName), "%s_y", "K_car");
        }
    } break;
    default: {
        // This is where bomb cars are handled but we do not implement them for Kinoko.
        break;
    }
    }

    m_initialScale = m_scale.x;
    CAR_SCALE = 1.5f * m_initialScale;
...
Kart::Reaction ObjectCarTGE::onCollision(Kart::KartObject *kartObj, Kart::Reaction reactionOnKart,
    ...
    if (m_highwayMgr) {
        ...
        if (m_highwayMgr->squashTimer() < SQUASH_INVULNERABILITY) {
            ...
            reactionOnKart = hitTable.reaction(hitTable.slot(static_cast<ObjectId>(3)));
        }
    }
...
bool ObjectCarTGE::checkCollision(ObjectCollisionBase *lhs, EGG::Vector3f &dist) {
    ...
    bool hasCol = lhs->check(*m_collision, dist); // hasCol should return false on 5868
...
void ObjectCarTGE::calcPos() {
    ...
    static u16 lastFrame = 0;
    static u16 idx = 0;
    u16 currentFrame = KTestSystem::Instance()->currentFrame();
    ...
    KTestSystem::Instance()->enableAlloc();
    if (idx == 18) {
        ; // REPORT("Frame %u Index %u: %s", currentFrame, idx, std::string(m_pos).c_str());
    }

    m_up = FUN_806B3CA4(m_tangent).base(1); // wrong for truck
