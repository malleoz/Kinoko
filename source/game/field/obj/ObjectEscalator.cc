...
ObjectEscalator::ObjectEscalator(const System::MapdataGeoObj &params, bool reverse /* = false*/)
    : ObjectKCL(params), m_initialPos(m_pos), m_initialRot(m_rot),
      m_dirChangeFrames(
              {static_cast<u32>(params.setting(2)) * 60, static_cast<u32>(params.setting(4)) * 60}),
      ...
      m_stopFrames({static_cast<f32>(m_dirChangeFrames[0]) - REVERSE_FRAMES_F32,
              static_cast<f32>(m_dirChangeFrames[1]) - REVERSE_FRAMES_F32}),
      m_startFrames({static_cast<f32>(m_dirChangeFrames[0]) + STANDSTILL_FRAMES,
              static_cast<f32>(m_dirChangeFrames[1]) + STANDSTILL_FRAMES}),
      m_fullSpeedFrames(
...
const EGG::Matrix34f &ObjectEscalator::getUpdatedMatrix(u32 timeOffset) {
    ...
    m_rtMat.makeRT(m_rot, m_initialPos + m_stepDims * calcStepFactor(t));
    return m_rtMat;
}
...
/// @addr{0x80800ABC}
f32 ObjectEscalator::calcStepFactor(u32 t) {
    ...
    auto dist0 = [this](u32 t) -> f64 {
        t = static_cast<u32>(std::clamp<s32>(t, 0, static_cast<s32>(m_stopFrames[0])));
    ...
    auto dist3 = [=, this](u32 t) -> f64 {
        t = static_cast<u32>(std::clamp<s32>(dtFullSpeed[0], 0, m_midDuration));
...
/// @addr{0x80800FBC}
f32 ObjectEscalator::calcSpeed(u32 t) {
    // Escalator has not stopped yet
    ...
    // Escalator is slowing down for the first time
    if (t < m_dirChangeFrames[0]) {
        return m_speed[0] * static_cast<f32>(m_dirChangeFrames[0] - t) / REVERSE_FRAMES_F32;
    }
    ...
    // Escalator is slowing down for the second time
    if (t < static_cast<u32>(m_dirChangeFrames[1])) {
        return m_speed[1] * static_cast<f32>(static_cast<u32>(m_dirChangeFrames[1]) - t) /
                REVERSE_FRAMES_F32;
    }
