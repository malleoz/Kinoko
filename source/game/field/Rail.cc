...
void Rail::checkSphereFull() {
    ...
    m_floorNrms.reserve(m_pointCount);
...
RailLine::RailLine(u16 idx, System::MapdataPointInfo *info) : Rail(idx, info) {
    ...
    m_transitions.resize(m_dirCount);
    m_pathLength = 0.0f;
...
const std::vector<f32> &RailLine::getPathPercentages() const {
    static std::vector<f32> EMPTY_PERCENTAGES(0);
