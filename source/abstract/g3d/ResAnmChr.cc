...
public:
    ...
    f32 GetValue(const ResAnmChr::FVSData *pFVSData) const {
        f32 x = parse<f32>(pFVSData->fvs32.scale);
        f32 y = static_cast<f32>(parse<u32>((mPtr->fvsU32 & 0x00FFF000) >> 12));
...
f32 CalcAnimationFVS(f32 frame, const ResAnmChr::FVSData *pFVSData) {
    ...
    using namespace EGG::Mathf;

    f32 frameDelta = frame - f0;
    f32 keyFrameDelta = f1 - f0;
    f32 keyFrameDeltaInv = finv(keyFrameDelta);
    f32 t = frameDelta * keyFrameDeltaInv;
    f32 tMinus1 = fms(frameDelta, keyFrameDeltaInv, 1.0f);
    f32 tanInterp = fma(tMinus1, t0, t * t1);
    f32 scaledCurve = t * (fms(2.0f, t, 3.0f) * (v0 - v1));

    return fma(frameDelta * tMinus1, tanInterp, fma(t, scaledCurve, v0));
}
